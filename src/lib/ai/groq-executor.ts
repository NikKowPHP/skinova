...
export async function executeGroqWithRotation<T>(
  ...
  for (const apiKey of allKeys) {
    try {
      ...
      if (
        ...
        logger.warn(
          `Groq API key failed (status: ${status}). Rotating to next key. Error: ${error.message}`,
        );
  ...
  throw new Error(
    `All Groq API keys failed. Last error: ${lastError?.message}`,
  );
}