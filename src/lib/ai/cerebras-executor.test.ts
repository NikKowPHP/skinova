...
// Mock dependencies
jest.mock("axios");
jest.mock("./cerebras-key-provider");
...
describe("executeCerebrasWithRotation", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it("should succeed on the first key", async () => {
    ...
    const result = await executeCerebrasWithRotation((client) =>
      client.post(""),
    );
  ...
  it("should throw an error if all keys fail", async () => {
    ...
    await expect(
      executeCerebrasWithRotation((client) => client.post("")),
    ).rejects.toThrow(
      "All Cerebras API keys failed. Last error: Internal Server Error",
    );
