  ...
  const decryptedData = scans.map(scan => {
    try {
      ...
      if (decryptedScan.analysis) {
        decryptedScan.analysis.analysisJson = decrypt(decryptedScan.analysis.analysisJson) ?? "DECRYPTION_FAILED";
        decryptedScan.analysis.rawAiResponse = decrypt(decryptedScan.analysis.rawAiResponse) ?? "DECRYPTION_FAILED";
      }
